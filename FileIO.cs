...
public class FileIO
{
    ...
    public static FromSoftFile<T>? OpenFile<T>(string dialogFilter) where T : SoulsFile<T>, new()
    {
        string filePath = ShowOpenFileDialog(dialogFilter);
        T? fileData = GetFileData<T>(filePath);
        return fileData == null ? null : new FromSoftFile<T>(filePath);
    }
    ...
    {
        FromSoftFile<MSBE>? msb = OpenFile<MSBE>(@"MSB File (*.msb, *.msb.dcx)|*.msb;*.msb.dcx");
